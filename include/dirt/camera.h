...
public:
    ...
    Camera(const json & j = json())
    {
        ...
        auto vfovrad_2=deg2rad(vfov)/2;
        auto h = 2 * tan(vfovrad_2);// *m_focalDistance;
        auto w = h * m_resolution.x / m_resolution.y;
        m_size = Vec2f(w, h);
    }
    ...
    Ray3f generateRay(float u, float v) const
    {
        ...
        Vec3f rayDirection;
        rayDirection.x = (u / m_resolution.x) * m_size.x - m_size.x / 2;
        rayDirection.y = (1.-v / m_resolution.y) * m_size.y - m_size.y / 2;
        rayDirection.z = -1.0f;
        return m_xform.ray(Ray3f(Vec3f::Zero(), rayDirection));
    }
