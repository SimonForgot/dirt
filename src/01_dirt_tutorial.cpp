	...
	for (auto y : range(rayImage.height()))
	{
		for (auto x : range(rayImage.width()))
		{
			...
			/*	const Vec3f cameraOrigin(0.f, 0.f, 0.f);
				const float imagePlaneWidth = 4.f;
				const float imagePlaneHeight = 2.f;*/
			rayDirection.x = (x / rayImage.width()) * imagePlaneWidth - imagePlaneWidth / 2;
			rayDirection.y = (y / rayImage.height()) * imagePlaneHeight - imagePlaneHeight / 2;
			rayDirection.z = -1.0f;
...
void testCameraClassImage()
{
	...
	string filename("D:/workspace/dirt_basecode/scenes/01_raytrace/01_camera_ray_image.png");
	message("Saving ray image to %s....\n", filename);
	rayImage.save(filename);
}
...
void testXformedCameraImage()
{
	...
	string filename("D:/workspace/dirt_basecode/scenes/01_raytrace/01_xformed_camera_ray_image11.png");
	message("Saving ray image to %s....\n", filename);
	rayImage.save(filename);
}
...
void testSphereImage()
{
	...
	string filename("D:/workspace/dirt_basecode/scenes/01_raytrace/01_xformed_camera_sphere_image.png");
    message("Saving ray image to %s....\n", filename);
    rayImage.save(filename);
}
...
void testRecursiveRaytracing()
{
	...
	Camera camera({
		              ...
		              //{"resolution", Vec2i(rayImage.sizeX(), rayImage.sizeY())},
		              {"fdist", 800.f},
		              {"transform", {{"from", {0, 20, 1077.5}},
		                             {"to",   {0, -4, 0}},
		                             {"up",   {0.0, 1.0,  0.0}}} },
	              });

    auto groundMaterial = make_shared<Lambertian>(json{{"albedo", Vec3f(0.73, 0.73, 0.73)}});
    auto matteMaterial =
        make_shared<DiffuseLight>(json{{"emit", 15}});
    //auto shinyMaterial = make_shared<Metal>(
        //json{{"albedo", Vec3f(1.0f, 1.0f, 1.0f)}, {"roughness", 0.3f}});
	//auto refractedMaterial = make_shared<Dielectric>(json{ {"ior",1.2f} });

	auto dielectricSphere = make_shared<Quad>(Vec2f(550, 550), groundMaterial,
		Transform::translate({ 0, 0, -277.5 }));
       // 1.f, refractedMaterial, Transform::translate({3.f, 1.f, 0.f}));
    auto shinySphere = make_shared<Quad>(Vec2f(130, 130), matteMaterial,
   ...
   // auto groundSphere = make_shared<Sphere>(
	...
	scene.addChild(dielectricSphere);
	...
	string filename("D:/workspace/dirt_basecode/scenes/01_raytrace/01_recursive_raytracing.png");
