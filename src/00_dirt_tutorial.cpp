...
void testVectorsAndMatrices()
{
	...
	float degrees = rad2deg(acos(dot(v1,v3)/(length(v1)*length(v3))));
...
void testColorAndImage()
{
    ...
    Color3f pinkish=0.5*red+0.5*white;
	...
	for(auto i=0;i<gradient.width();i++)
	for(auto j=0;j<gradient.height();j++)
	{
		gradient(i,j)=Color3f(i/200.0f,j/100.0f,0.0f);
	}
	...
	image.load("../scenes/00_preliminaries/cornellbox.png");
    // putYourCodeHere(
	...
	image.save("cornell_grayscale.png");
	success("Done!\n");
}
